 ...
 * <p>
 * Created on October 01, 2015 at 2:18 PM.
 */
...
class MyCookieStore implements CookieStore {
    ...
    public void add(URI uri, HttpCookie cookie) {
        if(uri == null) return;
        System.out.println("MyCookieStore:: add: (" + uri + ", " + cookie.toString() + ")");
    ...
    public MyCookieStore(Context ctxContext) {
        this.context = ctxContext;
        spePreferences = this.context.getSharedPreferences("CookiePrefsFile", 0);
        ...
        for(Map.Entry<String, ?> entry : prefsMap.entrySet()) {
            for (String strCookie : (HashSet<String>) entry.getValue()) {
                ...
                System.out.println("MyCookieStore:: " + entry.getKey() + ": " + strCookie);
            }
        }
    }
    ...
    public List<HttpCookie> get(URI uri) {
        ...
        if (lstCookies == null) mapCookies.put(uri, new ArrayList<HttpCookie>());
    ...
    public boolean remove(URI uri, HttpCookie cookie) {
        List<HttpCookie> lstCookies = mapCookies.get(uri);
        if (lstCookies == null) return false;
        return lstCookies.remove(cookie);
    }

    public Map<URI, List<HttpCookie>> getMapCookies() {
        return mapCookies;
    }
