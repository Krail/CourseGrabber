...
public class Grabber {
    ...
    public static int grab(Context context, File file, String username, String password, boolean csv) {
        ...
        if ((urlParamsAndCookie = Grabber.initialGet(username, password)) != null) {
            String[] locationAndCookie;
            if ((locationAndCookie = Grabber.initialPost(urlParamsAndCookie[0], urlParamsAndCookie[1])) != null) {
                if ((locationAndCookie = Grabber.redirect(locationAndCookie[0], locationAndCookie[1], 1)) != null) {
                    if ((locationAndCookie = Grabber.redirect(locationAndCookie[0], locationAndCookie[1], 2)) != null) {
                        if ((locationAndCookie = Grabber.redirect(locationAndCookie[0], locationAndCookie[1], 3)) != null) {
                            if ((locationAndCookie = Grabber.redirect(locationAndCookie[0], locationAndCookie[1], 4)) != null) {
                                String cookie = locationAndCookie[1];
                                if ((locationAndCookie = Grabber.redirect(locationAndCookie[0], locationAndCookie[1], 5)) != null) {
                                ...
                                } else return ERR_REDIRECT_5; // Failed 5th and final redirect (my code is wrong)
    ...
    private static String[] redirect(String location, String cookie, int num) {
        ...
        try {
            ...
            HttpsURLConnection conn = (HttpsURLConnection) url.openConnection();
            Log.i(TAG, "Redirect #" + num + ": ConnLocation=" + location);

            conn.addRequestProperty("User-Agent", "CourseGrabber");
            if (cookie != null && num == 1) conn.addRequestProperty("Cookie", cookie);
            Log.i(TAG, "Redirect #" + num + ": Cookie=" + cookie);

            conn.connect();


            locationAndCookies[0] = conn.getHeaderField("Location");
            Log.i(TAG, "Redirect #" + num + ": RedirectLocation=" + locationAndCookies[0]);
            if (conn.getHeaderFields().get("Set-Cookie") != null && num == 1) {
                Log.i(TAG, "Redirect #" + num + ": Set-Cookie=" + conn.getHeaderFields().get("Set-Cookie").get(0));
                locationAndCookies[1] = conn.getHeaderFields().get("Set-Cookie").get(0).split(";")[0];
                for (int i = 1; i < conn.getHeaderFields().get("Set-Cookie").size(); i++) {
                    Log.i(TAG, "Redirect #" + num + ": Set-Cookie=" + conn.getHeaderFields().get("Set-Cookie").get(i));
            ...
            int responseCode = conn.getResponseCode();
            conn.disconnect();
            Log.i(TAG, "Redirect #" + num + ": Response Code=" + responseCode);
